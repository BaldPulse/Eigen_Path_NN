...
def get_sgc_model(num_nodes=41, num_sgc_feats=32, latent_size=1):
    ...
    decode = sgc_decoder(num_nodes, name = 'decoder', kernel_regularizer = l1l2_corr_sm(l1 = 0.0, l2 = 0., kc = 0.0, ks = 0.2, kv = 0.2))(latent)
...
val_flows, val_noiseless_flows, _, _ = load_data(val_path)
flows = np.expand_dims(flows, 2)
val_flows = np.expand_dims(val_flows, 2)
...
model.fit(
    ...
    epochs=n_epochs,
    #callbacks=[tensorboard_callback, tbi_callback, time_callback],
    #callbacks=[time_callback],
    #validation_data=({'input_node_features': val_flows, 'adjacency': A},val_flows)
...
l_img = np.reshape(tf.nn.softmax(paths[0])*10, [1, e_shape[0], e_shape[1], 1])
l_path = os.path.join(log_dir, now.strftime("%H:%M:%S"))
